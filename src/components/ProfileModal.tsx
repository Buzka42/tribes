        ...
        <BlurView intensity={90} tint="dark" style={{ width: 320, padding: 24, borderRadius: 24, backgroundColor: Colors.bg, alignItems: "center", borderWidth: 1, borderColor: Colors.hairline }}>
          ...
          {profileViewData ? (
            <>
              ...
              <Text style={{ fontFamily: Typography.headline, fontSize: 22, color: Colors.textPrimary, marginBottom: 16 }}>{String(profileViewData.displayName || t('common.unknown'))}</Text>
              <View style={{ flexDirection: "row", gap: 8, marginBottom: 16, flexWrap: "wrap", justifyContent: "center" }}>
                {profileViewData.dateOfBirth && !isNaN(new Date(String(profileViewData.dateOfBirth)).getTime()) ? (
                  <View style={{ backgroundColor: Colors.bgInput, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, borderWidth: 1, borderColor: Colors.hairline }}>
                    <Text style={{ fontFamily: Typography.bodyBold, fontSize: 12, color: Colors.textSecondary }}>{tn('profile.years', Math.floor((Date.now() - new Date(String(profileViewData.dateOfBirth)).getTime()) / 31557600000))}</Text>
                  </View>
                ) : null}
                {profileViewData.sex ? (
                  <View style={{ backgroundColor: Colors.bgInput, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, borderWidth: 1, borderColor: Colors.hairline }}>
                    <Text style={{ fontFamily: Typography.bodyBold, fontSize: 12, color: Colors.textSecondary }}>
                      {{ Male: t('setup.male'), Female: t('setup.female'), Other: t('setup.other') }[String(profileViewData.sex)] || String(profileViewData.sex)}
                    </Text>
                  </View>
                ) : null}
              </View>
              ...
              <View style={{ flexDirection: "row", gap: 20, borderTopWidth: 1, borderTopColor: Colors.hairline, paddingTop: 16, width: "100%", justifyContent: "center", alignItems: "center" }}>
                <View style={{ alignItems: "center" }}>
                  <Text style={{ fontSize: 13, fontFamily: Typography.bodySemibold, color: Colors.textSecondary }}>{t('profile.hostRating')}</Text>
